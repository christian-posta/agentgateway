...
mod tests {
	...
	use crate::http::aauth::{
		AAuth, AAuthPolicyError, ChallengeConfig, JwksCache, Mode, RequiredScheme,
	};
	...
	fn make_min_req_log() -> RequestLog {
		...
		use crate::telemetry::{log, metrics::Metrics, trc};
	...
	fn test_jwks_cache_insert_and_get() {
		...
		// Retrieve keys
		assert_eq!(
			cache.get(agent_id, kid1).unwrap().kid,
			Some(kid1.to_string())
		);
		assert_eq!(
			cache.get(agent_id, kid2).unwrap().kid,
			Some(kid2.to_string())
		);
