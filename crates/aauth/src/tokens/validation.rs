...
pub fn decode_jwt_header(jwt: &str) -> Result<jsonwebtoken::Header, AAuthError> {
	decode_header(jwt)
		.map_err(|e| AAuthError::JwtValidationError(format!("invalid JWT header: {}", e)))
}
...
pub fn decode_jwt_claims_unverified(jwt: &str) -> Result<Map<String, Value>, AAuthError> {
	...
	let payload_bytes =
		base64::Engine::decode(&base64::engine::general_purpose::URL_SAFE_NO_PAD, parts[1]).map_err(
			|e| AAuthError::JwtValidationError(format!("invalid JWT payload encoding: {}", e)),
		)?;
...
pub fn validate_jwt(
	...
	let (decoding_key, algorithms) =
		match signing_jwk.kty.as_str() {
			"OKP" => {
				// Ed25519/EdDSA
...
pub fn extract_cnf_jwk(claims: &Map<String, Value>) -> Result<JWK, AAuthError> {
	let cnf = claims
		.get("cnf")
		.ok_or_else(|| AAuthError::JwtValidationError("missing cnf claim".to_string()))?;

	let cnf_obj = cnf
		.as_object()
		.ok_or_else(|| AAuthError::JwtValidationError("cnf claim is not an object".to_string()))?;

	let jwk_value = cnf_obj
		.get("jwk")
		.ok_or_else(|| AAuthError::JwtValidationError("missing cnf.jwk claim".to_string()))?;
